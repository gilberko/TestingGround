...
fun FoodScreen(navController: NavController) {
    Scaffold(
        ...
        Column(
            ...
            FORow("krajíc / plátek chleba", "slice of bread", "krajíc (m.) = thick slice; plátek (m.) = thin slice")
            FORow("česnekový chléb", "garlic bread", "m. — also: topinka s česnekem (garlic toast)")
            ...
            FORow("smetanový sýr", "cream cheese", "m. — Philadelphia-type; also just called 'Philadelphia' in everyday speech")
            FORow("margarín", "margarine", "m.")

            FOSection("Eggs (Vejce)")
            FORow("vejce", "egg", "n. — pl. also vejce (same form); e.g. dvě vejce = two eggs")
            FORow("vejce naměkko", "soft-boiled egg", "naměkko = soft; the yolk remains runny")
            FORow("vejce natvrdo", "hard-boiled egg", "natvrdo = hard; fully cooked through")
            FORow("míchaná vejce", "scrambled eggs", "pl., n. — míchaná = mixed/scrambled")
            FORow("volské oko", "fried egg / sunny side up", "n. — lit. 'bull's eye'; the classic round fried egg with unbroken yolk")
            FORow("vejce pošírovaná", "poached eggs", "pl. — pošírovaná = poached; cooked in simmering water without the shell")
            FORow("omeleta", "omelet / omelette", "f.")
            ...
            FORow("slanina", "bacon", "f. — smoked cured pork belly; špek (m.) is the older/regional variant")
            FORow("klobása / párek", "sausage / frankfurter", "klobása f. = thick sausage; párek m. = thin hot-dog-style sausage")
            FORow("hamburger", "hamburger", "m.")
            FORow("corned beef", "corned beef", "m. — borrowed term; Czech: hovězí solonina")
            FORow("rostbíf", "roast beef", "m. — from French rosbif; slow-roasted beef joint")
            ...
            FORow("mrkev", "carrot", "f.")
            FORow("lilek", "eggplant", "m. — British English: aubergine; also: baklažán")
            FORow("cuketa", "zucchini", "f. — British English: courgette")
            FORow("houba / žampion", "mushroom / button mushroom", "houba f. = mushroom (general); žampion m. = champignon/button mushroom; houby = wild mushrooms (foraging is very popular in Czech culture)")
            FORow("jarní cibulka", "green onion / spring onion", "f. — note: pažitka = chives, a different herb")
            FORow("řeřicha", "watercress", "f.")
            FORow("česnek", "garlic", "m.")
            ...
            FORow("limetka", "lime", "f.")
            FORow("grapefruit / grep", "grapefruit", "m. — grep is the common colloquial Czech form")
            FORow("klementinka", "clementine", "f.")
            FORow("hrozny", "grapes", "pl., m. — sg. hrozen (m.)")
            FORow("třešně", "cherries", "pl., f. — sg. třešeň (f.)")
            FORow("pomelo", "pomelo", "n.")
            ...
            FORow("špagety", "spaghetti", "pl., f. — těstoviny covers all pasta; špagety is this specific type")
            FORow("ravioli", "ravioli", "pl. — borrowed word; also: plněné těstoviny (stuffed pasta)")
            ...
            FORow("olej", "oil", "m. — olivový olej = olive oil; slunečnicový olej = sunflower oil")
            ...
            FORow("kousek dortu", "piece of cake", "kousek (m.) = piece/bit; dortu = genitive of dort")
            ...
            FORow("smažený / smažená / smažené", "fried  (pan-fried)", "cooked in a pan with oil or fat")
            FORow("fritovaný / fritovaná / fritované", "deep-fried", "submerged in hot oil; from 'fritura'")
            FORow("vařený / vařená / vařené", "boiled", "cooked in boiling water")
            FORow("pošírovaný / pošírovaná / pošírované", "poached", "cooked gently in simmering liquid")
            FORow("šejkovaný", "shaken", "cocktail term — mixed by shaking with ice")
            FORow("míchaný", "stirred", "cocktail term — mixed by stirring; also general: mixed")
