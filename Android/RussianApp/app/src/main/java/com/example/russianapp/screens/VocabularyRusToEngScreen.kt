...
private fun buildRusToEngQuestions(
    ...
    return selected.map { entry ->
        val distractors = pool
            .filter { it.id != entry.id && it.english != entry.english }
            .shuffled(rng)
            .take(3)
            .map { it.english }
        QuizQuestion(
            promptWord = entry.russian,
            promptLabel = "Russian → English",
            questionText = "What is the English meaning?",
...
@Composable
fun VocabularyRusToEngScreen(configViewModel: ConfigViewModel, onBack: () -> Unit) {
    ...
    if (rawJson != null && quizState.questions.isEmpty()) {
        ...
        quizState = QuizState(questions = buildRusToEngQuestions(file.vocabulary, numQuestions))
    }
...
@Composable
private fun VocabRusToEngScoreScreen(
    score: Int,
    total: Int,
    onRetry: () -> Unit,
    onBack: () -> Unit,
