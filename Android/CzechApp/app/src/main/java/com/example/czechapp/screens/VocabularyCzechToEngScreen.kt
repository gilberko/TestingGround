...
private fun buildCzechToEngQuestions(entries: List<VocabularyEntry>, count: Int, rng: Random = Random.Default): List<QuizQuestion> {
    val pool = entries.shuffled(rng)
    val selected = pool.take(count.coerceAtMost(pool.size))
    return selected.map { entry ->
        val distractors = pool.filter { it.id != entry.id && it.english != entry.english }
            .shuffled(rng).take(3).map { it.english }
        QuizQuestion(
            promptWord = entry.czech, promptLabel = "Czech → English",
            questionText = "What is the English word?",
            correctAnswer = entry.english,
            choices = (distractors + entry.english).shuffled(rng)
        )
    }
}
...
@Composable
fun VocabularyCzechToEngScreen(configViewModel: ConfigViewModel, onBack: () -> Unit) {
    ...
    if (rawJson != null && quizState.questions.isEmpty()) {
        ...
        quizState = QuizState(questions = buildCzechToEngQuestions(file.vocabulary, numQuestions))
    }
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Vocabulary: Czech → Eng") },
        ...
        Box(modifier = Modifier.fillMaxSize().padding(innerPadding)) {
            when {
                ...
                quizState.isFinished -> VocabCzechScoreScreen(
...
@Composable
private fun VocabCzechScoreScreen(score: Int, total: Int, onRetry: () -> Unit, onBack: () -> Unit, modifier: Modifier = Modifier) {
