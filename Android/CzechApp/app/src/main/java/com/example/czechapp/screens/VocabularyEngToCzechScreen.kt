...
import com.example.czechapp.R
import com.example.czechapp.data.QuizQuestion
import com.example.czechapp.data.QuizState
import com.example.czechapp.data.VocabularyEntry
import com.example.czechapp.data.VocabularyQuizFile
import com.example.czechapp.viewmodel.ConfigViewModel
import com.example.czechapp.viewmodel.QuizMode
import com.google.gson.Gson
import kotlinx.coroutines.Dispatchers
import kotlinx.coroutines.withContext
import kotlin.random.Random

private fun buildEngToCzechQuestions(entries: List<VocabularyEntry>, count: Int, rng: Random = Random.Default): List<QuizQuestion> {
    val pool = entries.shuffled(rng)
    val selected = pool.take(count.coerceAtMost(pool.size))
    return selected.map { entry ->
        val distractors = pool.filter { it.id != entry.id && it.czech != entry.czech }
            .shuffled(rng).take(3).map { it.czech }
        QuizQuestion(
            promptWord = entry.english, promptLabel = "English → Czech",
            questionText = "What is the Czech word?",
            correctAnswer = entry.czech,
            choices = (distractors + entry.czech).shuffled(rng)
        )
    }
}
...
@Composable
fun VocabularyEngToCzechScreen(configViewModel: ConfigViewModel, onBack: () -> Unit) {
    ...
    if (rawJson != null && quizState.questions.isEmpty()) {
        ...
        quizState = QuizState(questions = buildEngToCzechQuestions(file.vocabulary, numQuestions))
    }
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Vocabulary: Eng → Czech") },
        ...
        Box(modifier = Modifier.fillMaxSize().padding(innerPadding)) {
            when {
                rawJson == null -> CircularProgressIndicator(modifier = Modifier.align(Alignment.Center))
                quizState.isFinished -> VocabScoreScreen(
                    score = quizState.score, total = quizState.questions.size,
                    onRetry = { rebuild() }, onBack = onBack,
                    ...
                    if (question != null) {
                        LazyColumn(
                            ...
                            item {
                                Text(question.promptWord, style = MaterialTheme.typography.displaySmall,
                                    color = MaterialTheme.colorScheme.primary, fontWeight = FontWeight.Bold,
                                    textAlign = TextAlign.Center, modifier = Modifier.fillMaxWidth())
                                Text(question.promptLabel, style = MaterialTheme.typography.bodyMedium,
                            ...
                            if (quizMode == QuizMode.FREE_TEXT) {
                                item {
                                    ...
                                    OutlinedTextField(value = textInput, onValueChange = { if (!answered) textInput = it },
                                        label = { Text("Your answer") }, modifier = Modifier.fillMaxWidth(), enabled = !answered)
                                }
                                item {
                                    val answered = quizState.isAnswered
                                    Button(onClick = {
                                        if (!answered && textInput.isNotBlank()) {
                                            ...
                                            quizState = quizState.copy(selectedAnswer = textInput.trim(),
                                                score = if (isCorrect) quizState.score + 1 else quizState.score)
                                        }
                                    }, enabled = !answered && textInput.isNotBlank(), modifier = Modifier.fillMaxWidth()) { Text("Submit") }
                                }
                                ...
                                items(question.choices.size) { i ->
                                    ...
                                    val contentColor = if (answered && (isCorrect || isSelected)) Color.White else MaterialTheme.colorScheme.onSurface
                                    OutlinedButton(
                                        onClick = {
                                            if (!answered) quizState = quizState.copy(selectedAnswer = choice,
                                                score = if (isCorrect) quizState.score + 1 else quizState.score)
                                        },
                            ...
                            if (quizState.isAnswered) {
                                item {
                                    Button(onClick = {
                                        ...
                                        quizState = if (nextIndex >= quizState.questions.size) quizState.copy(isFinished = true)
                                        else quizState.copy(currentIndex = nextIndex, selectedAnswer = null)
...
@Composable
private fun VocabScoreScreen(score: Int, total: Int, onRetry: () -> Unit, onBack: () -> Unit, modifier: Modifier = Modifier) {
    Column(modifier = modifier.padding(32.dp), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(16.dp)) {
        ...
        Text("$score / $total correct", style = MaterialTheme.typography.displaySmall,
