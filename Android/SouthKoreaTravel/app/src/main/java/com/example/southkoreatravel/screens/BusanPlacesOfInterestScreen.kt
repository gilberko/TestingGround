...
fun BusanPlacesOfInterestScreen(onBack: () -> Unit) {
    ScreenScaffold(title = "Places Of Interest", onBack = onBack) {
        Column(
            ...
            SectionHeader("ARTE MUSEUM")
            BodyText(boldNames("ARTE MUSEUM, in Yeongdo-gu on Yeongdo Island next to the P.ARK complex, is an immersive digital/media art museum by d'strict — the same studio behind Seoul's WAVE billboard at COEX. This is the largest ARTE MUSEUM location in the world at 5,600 square meters, built around the theme \"Eternal Nature\": large-scale projection rooms simulating waterfalls, gardens, and oceans rather than displaying static art. It suits all ages, with no restrictions — a popular family and photo-spot attraction.", listOf("ARTE MUSEUM", "d'strict")))
            BodyText(boldNames("It's open daily 10am-9pm, last admission 8pm. Tickets run roughly ₩22,000 adult / ₩18,000 youth / ₩15,000 child on weekdays, and ₩25,000 / ₩21,000 / ₩18,000 on weekends and holidays. To get there, take Line 1 to Nampo Station, Exit 8, walk to Yeongdodaegyo, then city bus 17 or 186 to the \"Arte Museum Busan\" stop. Book in advance on the official site (artemuseum.com) or through resellers like Trazy, KKday, or Trip.com — walk-up tickets at the counter are also available, but advance booking is recommended, especially on weekends.", listOf("ARTE MUSEUM")))
        }
    }
}
