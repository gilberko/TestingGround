...
fun MedicalIssuesScreen(onBack: () -> Unit) {
    ScreenScaffold(title = "Medical Issues", onBack = onBack) {
        Column(
            ...
            BodyText("For less urgent medical questions — unsure whether something needs a hospital tonight or can wait for a clinic tomorrow — call 1339, a 24/7 medical consultation line. There's also 1330, the Korea Tourism Organization's tourist hotline, useful for general travel problems and interpretation, not just medical ones.")
            BodyText("For English-speaking hospital care, major hospitals with dedicated international clinics include Severance Hospital (Yonsei University) and Seoul National University Hospital's International Health Center, both in Seoul, with English-speaking staff and (at Severance) 24-hour emergency contact in English. For dental emergencies, several Seoul clinics specialize in treating international patients with English-speaking dentists — look for ones explicitly advertising foreigner/international patient services rather than a random local clinic, since English support varies a lot outside of those dedicated practices.")
        }
    }
}
